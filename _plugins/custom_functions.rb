module Jekyll
    ...
    module MyCustomFilters
      ...
      def get_labs_by_module(page, site)
        labs = []
        if page && page['labs'] # Ensure page['labs'] is not nil before using it
            labs = site['assignments'].select { |item| page['labs'].include?(item['num']) && item['type'] == 'lab' }
        end
        return labs
      end

      def get_lectures_by_module(page, site)
        labs = []
        if page['lectures'] # Ensure page['labs'] is not nil before using it
            labs = site['lectures'].select { |item| page['lectures'].include?(item['num']) && item['type'] == 'lecture' }
        end
        return labs
      end
      ...
      def get_projects_by_module(page, site)
        projects = []
        if page && page['projects'] # Ensure page['labs'] is not nil before using it
